...
void TIM3DelayUs_Init(u16 nus)
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	//定时器TIM3初始化
	TIM_TimeBaseStructure.TIM_Period = nus; //设置在下一个更新事件装入活动的自动重装载寄存器周期的值	
	TIM_TimeBaseStructure.TIM_Prescaler = (36-1); //设置用来作为TIMx时钟频率除数的预分频值 1mhz
	TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1; //设置时钟分割:TDTS = Tck_tim
