...
{ 										  
  ...
  FSMC_NORSRAMInitStructure.FSMC_DataAddressMux = FSMC_DataAddressMux_Disable; // 不复用数据地址（数据线与地址线不复用）
  FSMC_NORSRAMInitStructure.FSMC_MemoryType =FSMC_MemoryType_SRAM;// FSMC_MemoryType_SRAM;  存储器类型SRAM   
  FSMC_NORSRAMInitStructure.FSMC_MemoryDataWidth = FSMC_MemoryDataWidth_16b;//存储器数据宽度为16bit   
  FSMC_NORSRAMInitStructure.FSMC_BurstAccessMode =FSMC_BurstAccessMode_Disable;// FSMC_BurstAccessMode_Disable; 使用异步写模式，禁止突发模式
  ...
  FSMC_NORSRAMCmd(FSMC_Bank1_NORSRAM4, ENABLE);  // 使能BANK1 
	...
	{	
 	...
 	printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID 
	//USART2_printf(USART2, lcddev.id);
	...
	}else if(lcddev.id==0x9325)//9325
	{
		...
		LCD_WriteReg(0x000A,0x0000); //displat setting
		LCD_WriteReg(0x000C,0x0000); //displat setting
		LCD_WriteReg(0x000D,0x0000);
		LCD_WriteReg(0x000F,0x0000);
		//power on sequence VGHVGL电源配置
		...
		LCD_WriteReg(0x0051,0x00EF); //水平GRAM起始位置
		LCD_WriteReg(0x0052,0x0000); //垂直GRAM起始位置
		LCD_WriteReg(0x0053,0x013F);  //垂直GRAM起始位置
